...
def run_diff(config: ImprintConfig) -> None:
    ...
    current_dotfiles = dotfiles_collector.collect(
        config.home_dir
    )
    ...
    current_exts = set(
        current_vscode.get("extensions", [])
    )
    saved_exts = set(
        manifest.vscode.get("extensions", [])
    )
    ...
    current_pip = set(
        current_packages.get("pip", {})
        .get("packages", [])
    )
    saved_pip_d = manifest.packages.get("pip", {})
    saved_pip = set(
        saved_pip_d.get("packages", [])
        if isinstance(saved_pip_d, dict) else []
    )
    ...
    current_npm = set(
        current_packages.get("npm", {})
        .get("packages", [])
    )
    saved_npm_d = manifest.packages.get("npm", {})
    saved_npm = set(
        saved_npm_d.get("packages", [])
        if isinstance(saved_npm_d, dict) else []
    )
    ...
    add_count = (
        len(added_df) + len(added_ext)
        + len(added_pip) + len(added_npm)
    )
    rem_count = (
        len(removed_df) + len(removed_ext)
        + len(removed_pip) + len(removed_npm)
    )
    ...
    if total_changes > 0:
        ...
        if add_count:
            parts.append(
                f"[imp.add]+{add_count} added[/imp.add]"
            )
        if rem_count:
            parts.append(
                f"[imp.remove]-{rem_count} "
                f"removed[/imp.remove]"
            )
        mod_count = total_changes - add_count - rem_count
        if mod_count:
            parts.append(
                f"[imp.modify]~{mod_count} "
                f"modified[/imp.modify]"
            )
        ...
        console.print(
            "  [dim]Run[/dim]  "
            "[imp.cmd]imp update[/imp.cmd]  "
            "[dim]to save these changes to "
            "your snapshot.[/dim]"
        )
    else:
        console.print(
            "  [imp.success]✓[/imp.success]  "
            "Everything is in sync!"
        )
        console.print(
            "  [dim]No changes since snapshot at"
            "[/dim]  "
            f"[imp.cmd]{snapshot_at}[/imp.cmd]"
        )
